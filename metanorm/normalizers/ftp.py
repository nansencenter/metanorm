...
class FTPMetadataNormalizer(BaseMetadataNormalizer):
    ...
    def match_domain(self, raw_attributes):
        ...
        if set(['url']).issubset(raw_attributes.keys()):
            if urlparse(raw_attributes['url']).netloc in self.domain_set:
                return urlparse(raw_attributes['url']).netloc
    ...
    def get_platform(self, raw_attributes):
        ...
        platform_on_ftp = {"ftp://ftp.remss.com/gmi": 'GPM',
                           ...
                           "ftp://ftp.gportal.jaxa.jp/standard/GCOM-W/GCOM-W.AMSR2": 'GCOM-W1'}
        for ftp_base_dir in platform_on_ftp.keys():
            if raw_attributes['url'].startswith(ftp_base_dir):
    ...
    def get_time_coverage_start(self, raw_attributes):
        ...
        if match_result:
            if match_result == "ftp.remss.com":
                extracted_date = ftp_add_and_file_name.split(
                ...
                if 'd3d' in ftp_add_and_file_name.split('/')[-1]:
                ...
                return dateutil.parser.parse("19820101T000000").replace(tzinfo=tzutc())+relativedelta(days=+int(ftp_add_and_file_name.split('/')[-1][1:4]))
                ...
                return dateutil.parser.parse(ftp_add_and_file_name.split('/')[-1].split('_')[1]).replace(tzinfo=tzutc())
        else:
            return None
    ...
    def get_time_coverage_end(self, raw_attributes):
        ...
        if match_result:
            if match_result == "ftp.remss.com":
                ...
                return dateutil.parser.parse("20100101T000000").replace(tzinfo=tzutc())+relativedelta(days=+int(ftp_add_and_file_name.split('/')[-1][1:4]))
    ...
    def get_provider(self, raw_attributes):
        ...
        if match_result is None:
            return match_result
        return pti.get_gcmd_provider(provider_map[match_result])
