...
class FTPMetadataNormalizer(BaseMetadataNormalizer):
    ...
    # form it.
    domain_map = {"anon-ftp.ceda.ac.uk": r'esacci|aerosol|biomass|cloud|esacci_terms_and_conditions\.txt|fire|ghg|glaciers|ice_sheets_antarctica|ice_sheets_greenland|lakes|land_cover|ocean_colour|ozone|permafrost|sea_ice|sea_level|sea_state|sea_surface_salinity|snow|soil_moisture|sst',
                  "ftp.remss.com": r'amsr2|amsre|amsrj|ascat|ccmp|gmi|msu|nscat|qscat|seawinds|smap|ssmi|sst|support|TC-winds|tc_wakes|tmi|vapor|water_cycle|web\.config|welcome\.txt|wind|windsat',
                  "ftp.gportal.jaxa.jp": r'JERS-1|ADEOS-2|MOS-1b|SLATS|CIRC|MOS-1|AQUA|GCOM-C|GCOM-W|GSMaP|ADEOS|GPM|GPMConstellation|TRMM|TRMM_GPMFormat|GCOM-W\.AMSR2|L3\.PRC_10|L3\.SIC_10|L2\.TPW|L3\.TPW_25|L3\.CLW_10|L3\.SSW_10|L3\.TB36GHz_25|L3\.TB6GHz_10|L2\.SIC|L3\.TB89GHz_25|L2\.SSW|L3\.TB36GHz_10|L2\.PRC|L3\.SND_25|L3\.SMC_10|L3\.TB23GHz_10|L2\.SMC|L3\.TB6GHz_25|L3\.CLW_25|L3\.TB7GHz_10|L1B|L3\.SSW_25|L2\.CLW|L3\.SMC_25|L3\.SND_10|L3\.SST_25|L3\.PRC_25|L3\.TB10GHz_10|L3\.TB7GHz_25|L2\.SST|L3\.SIC_25|L3\.TB18GHz_10|L3\.SST_10|L3\.TB89GHz_10|L1R|L3\.TB23GHz_25|L3\.TB18GHz_25|L3\.TPW_10|L2\.SND|L3\.TB10GHz_25'}
    ...
    def match_domain(self, raw_attributes):
        ...
        if set([self.metadata_name]).issubset(raw_attributes.keys()):
            ...
            if raw_attributes['ftp_domain_name'] in domain_set:
    ...
    def dictionary_key_finder(self, raw_attributes, associated_dictionary):
        ...
        """
        pattern = re.compile(self.domain_map[raw_attributes['ftp_domain_name']])
    ...
    def get_platform(self, raw_attributes):
        ...
        dict_for_get_gcmd_platform = {"esacci": 'Earth Observation Satellites',  # ceda
                                      "gmi": 'GPM',  # remss
                                      "GCOM-W": 'GCOM-W1'}  # jaxa
        if self.match_domain(raw_attributes) is None:
            return self.match_domain(raw_attributes)
    ...
    def get_time_coverage_start(self, raw_attributes):
        ...
        if self.match_domain(raw_attributes):
            if self.match_domain(raw_attributes) == "ftp.remss.com":
            ...
            elif self.match_domain(raw_attributes) == "anon-ftp.ceda.ac.uk":
            ...
            elif self.match_domain(raw_attributes) == "ftp.gportal.jaxa.jp":
    ...
    def get_location_geometry(self, raw_attributes):
        ...
        if domain_str is None:
            return None
        elif domain_str == "anon-ftp.ceda.ac.uk" or "ftp.remss.com":
